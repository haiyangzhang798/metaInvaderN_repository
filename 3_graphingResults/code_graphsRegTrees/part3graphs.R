...
#########################################################
#(P3) Do native community vals (x) predict the magnitude that soils 'change' due to invasion (y)?
#(P3A) soil resps 1-3
#x
xmat<-natCWMcalc
xaxlab<-'Native CWM trait value'
...
mtext('Native community trait values',side=1,line=4,outer=T) # x label
...
#write tables
write.csv(summats.df,file='P3Asum.csv')
write.csv(coeftabs.df,file='P3Acoef.csv')
...
#write tables
write.csv(summats.df,file='P3Bsum.csv')
write.csv(coeftabs.df,file='P3Bcoef.csv')
